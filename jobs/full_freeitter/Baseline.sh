...
#SBATCH --job-name=Baseline_full_freeitter
...
for shot in 1 5; do
    ...
    python run_trainer.py --shot_num $shot --train_episode 10000 --training_test_episode 5000 --epoch 100 --test_epoch 5 --tag freeitter --conf_file ./config/baseline.yaml --data_root ./dataset/miniImageNet--ravi
    mv $SLURM_TMPDIR/LibFewShot/temp/* ~/scratch/LibFewShot/results/

    python run_trainer.py --shot_num $shot --train_episode 10000 --training_test_episode 5000 --epoch 100 --test_epoch 5 --tag freeitter --conf_file ./config/baseline.yaml --data_root ./dataset/tiered_imagenet
    mv $SLURM_TMPDIR/LibFewShot/temp/* ~/scratch/LibFewShot/results/
done
